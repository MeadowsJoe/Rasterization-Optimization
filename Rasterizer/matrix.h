...
public:
    ...
    //unrolled Matrix matrix multiplication
    matrix operator * (const matrix& mx) const {
        matrix ret;
        ret.a[0 * 4 + 0] = 
            a[0 * 4 + 0] * mx.a[0 * 4 + 0] +
    ...
    // - aRad: Rotation angle in radians
    // Returns the rotation matrix
    static matrix makeRotateZ(float& sinz, float& cosz) {
        matrix m;
        m.identity();
        m.a[0] = cosz;
        m.a[1] = -sinz;
    ...
    static matrix makeRotateX(float& sinx, float& cosx) {
        matrix m;
        m.identity();
        m.a[5] = cosx;
        m.a[6] = -sinx;
    ...
    static matrix makeRotateY(float& siny, float& cosy) {
        matrix m;
        m.identity();
        m.a[0] = cosy;
        m.a[2] = siny;
